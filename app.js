...
/* === AJAX submit for Web3Forms (single-bind + single-send guard, status above button) === */
document.addEventListener('DOMContentLoaded', () => {
  const form = document.getElementById('rq-form');
  if (!form) return;

  if (form.dataset.bound === '1') return; // avoid duplicate bind
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (form.dataset.sending === '1') return; // prevent double submit
